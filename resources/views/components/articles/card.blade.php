    ...
    <div class="px-1.5 pb-1 pt-2.5">
        <div class="flex flex-wrap items-center justify-between gap-5">
            <div class="flex select-none items-center justify-center gap-2 rounded-full py-2.5 pl-4 pr-5 text-xs"
                x-bind:class="{
                    'bg-purple-100/80 text-purple-700': types[article.type].color === 'purple',
                ...
                {{-- Type Icon --}}
                <div x-html="types[article.type].icon" class="-my-4"
                    x-bind:class="{
                        'text-purple-500': types[article.type].color === 'purple',
            ...
            <div class="flex items-center gap-1.5 pr-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="text-peach-orange" width="20" height="20"
                    viewBox="0 0 24 24">
                ...
                <div class="pt-0.5 text-sm font-medium text-dolphin" x-text="article.stars_count"></div>
            </div>
        </div>

        {{-- Title --}}
        <div x-text="article.title" class="line-clamp-2 pt-3.5 text-base font-medium text-evening"></div>
        ...
        <div class="flex items-center gap-3 pt-3.5">
            <div class="h-9 w-9 shrink-0 overflow-hidden rounded-full">
                <template x-if="article.author.avatar">
                    <div x-bind:style="'background-image: url(' + article.author.avatar + ')'"
                        class="aspect-square h-full w-full bg-cover bg-center bg-no-repeat"></div>
                </template>
                <template x-if="! article.author.avatar">
                    <div class="grid h-full w-full place-items-center bg-fair-pink text-salmon/50">
            ...
            <div class="space-y-0.5 text-xs">
                <div class="font-semibold capitalize" x-text="article.author.name"></div>
                <div class="text-dolphin/80" x-text="article.publish_date"></div>
            </div>
        </div>
